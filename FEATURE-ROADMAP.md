...
The managed-emulator matrix now executes deterministic runtime acceptance on Android 11 / API 30 and Android 14 / API 34. The API 34 lane confirms modern `CallEndpoint`/`InCallService` API presence, full-screen-intent capability probing, and continued fail-closed role consent. This is platform/runtime evidence only and does not prove live carrier calls, OEM behavior, real endpoint switching, or production default-phone replacement.
